//Задача 6.
//
//Создать приложение, в котором пользователь вводит две различных цифры. На выходе приложение выдает, если это возможно,
// из введенных цифр, нечетное число. Результат выводится в консоль. П
// ри невозможности создать нечетное число выводится сообщение – «Создать нечетное число невозможно».
// Каждое число вводится на отдельной строке
fun main(){
    println("Введите цифру от 0 до 9 ")
    val a:Byte = readln().toByte()
    println("Введите второе цифру от 0 до 9 ")
    val b:Byte= readln().toByte()
    if(a%2!=0) {
        println(b.toString()+a.toString())
    } else if(b%2!=0){
        println(a.toString()+b.toString())
    } else
        println("Создать нечетное чило невозожно")
}